 ...
 * @brief Implementare l'algoritmo di Bellman-Ford, sfruttando il codice di shortest path
 * e adattare le visualizzazioni dei grafi con dot per mostrare il suo corretto funzionamento
 */
...
/// @brief trova e memorizza, per ogni nodo del grafo, il percorso migliore partendo da source:n
/// restituisce false in caso di cicli negativi
    ...
    */
    ...
    for (int i = n; i < n_nodi - 1; i++)
    {
        ...
        for (int j = n; j < n_nodi; j++)
        {
            ...
            while (V_dist[j] != INFTY && elem != NULL)
            {
                int v = elem->val; // arco u-->v

                float alt = V_dist[j] + elem->w; /// costo per arrivare al nuovo nodo passando per u
                if (alt < V_dist[v])
    ...
    for (int i = n; i < n_nodi - 1; i++)
    {
        for (int j = n; j < n_nodi; j++)
        {
            ...
            while (V_dist[j] != INFTY && elem != NULL)
            {
                ...
                if (alt < V_dist[v])
                { // il percorso sorgente-->u-->v migliora il percorso attuale sorgente-->v
                    // possibile solo in caso di presenza di cicli negativi
                    return false;
                }
                elem = elem->next;
            }
...
int main(int argc, char **argv)
{
    ...
    /// generazione grafo alternativa
    list_insert_front(E[0], 1, 7);
    list_insert_front(E[0], 2, 3);

    list_insert_front(E[1], 2, -4);
